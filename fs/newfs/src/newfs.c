...
struct newfs_super newfs_super; 					/* 内存中的超级块 */
/******************************************************************************
* SECTION: FUSE操作定义
*******************************************************************************/
...
int newfs_getattr(const char* path, struct stat * newfs_stat) {
	...
	newfs_stat->st_blksize = NFS_BLK_SZ(); /* 这里修改了 */

	if (is_root) {
		newfs_stat->st_size	= newfs_super.sz_usage; 
		newfs_stat->st_blocks = NFS_DISK_SZ() / NFS_BLK_SZ(); /* 这里修改了 */
...
int main(int argc, char **argv)
{
	...
	newfs_options.device = strdup("/home/guests/190110429/ddriver");
